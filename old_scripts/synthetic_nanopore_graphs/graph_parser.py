...
def from_csv(graph_path, reads_path):
    ...
    with open(graph_path) as f:
        for line in f.readlines():
            ...
            if flag == 0:
                ...
                _, idx, strand, start, end = description.split()

                idx = re.findall(r'idx=(\d+)', idx)
                if len(idx) == 0:
                    print("skipped line bc of ID")
                    continue
                else:
                    idx = int(idx[0])
                ...
                # -----------------------------------------
                start = re.findall(r'start=(\d+)', start)
                if len(start) == 0:
                    print("skipped line bc of start token")
                    continue
                else:
                    start = max(0,int(start[0]))

                end = re.findall(r'end=(\d+)', end)
                if len(end) == 0:
                    print("skipped line bc of end token")
                    end = start
                else:
                    end = max(0,int(end[0]))
                ...
                if src_id in read_length.keys():
                    overlap_l = read_length[src_id] - prefix_len
                else:
                    overlap_l = 0
                ...
                if (src_id, dst_id) not in prefix_length.keys():
                    ...
                    overlap_length[(src_id, dst_id)] = overlap_l
    ...
    #graph_pg = None #pg_utils.from_networkx(graph_nx)
    ...
    return graph_nx #, predecessors, successors, reads, edges

def from_csv_old(graph_path, reads_path):
    """Parse an assembly graph stored in a CSV format.
