...
def create_lengths_file(lengths, name):

    with open(f'{name}_lengths.txt', 'w') as f:
        for l in lengths:
            f.write(str(l) + "\n")

def main(args):
    filename = args.filename
    find_l_dist = args.length or args.length_file
    ...
    try:
        if filename[-2:] == 'gz':
            ...
            f = open(filename)
        if find_l_dist:
            lengths = np.array([len(s) for s in SeqIO.parse(f, 'fasta')])
            f.seek(0)
        if find_q_dist:
    ...
    if args.length_file:
        print('Create length file...')
        create_lengths_file(lengths, name)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--length_file', action='store_true', default=False)
